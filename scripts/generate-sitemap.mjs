...
function buildSitemap() {
  ...
  const xml = [
    ...
    `<!--`,
    `  FlyttGo sitemap — generated by scripts/generate-sitemap.mjs`,
    `  Run: npm run sitemap   (or via prebuild)`,
    `  Sources: src/components/seo/seo-content.ts       (corridors)`,
    `           src/lib/service-categories.ts            (services)`,
    `           src/lib/expansion-cities.ts              (anchor cities)`,
    `           src/lib/cross-border-corridors.ts        (cross-border corridors)`,
    `           static list maintained in the script itself`,
    `-->`,
    ...
    ...staticUrls,
    ``,
    `  <!-- Service category landing pages (${categoryUrls.length} entries) -->`,
    ...categoryUrls,
    ``,
    `  <!-- Per-corridor SEO landing pages (${corridorUrls.length} entries) -->`,
    ...corridorUrls,
    ``,
    `  <!-- Strategic-city landing pages (${movingCityUrls.length} anchor cities) -->`,
    ...movingCityUrls,
    ``,
    `  <!-- Cross-border corridor landing pages (${crossBorderUrls.length} entries) -->`,
    ...crossBorderUrls,
    ``,
    `</urlset>`,
    ``,
  ...
  return {
    xml,
    corridorCount:    corridorUrls.length,
    categoryCount:    categoryUrls.length,
    staticCount:      staticUrls.length,
    movingCityCount:  movingCityUrls.length,
    crossBorderCount: crossBorderUrls.length,
  };
}

const result = buildSitemap();
writeFileSync(resolve(ROOT, 'public/sitemap.xml'), result.xml, 'utf8');

console.log(`✓ sitemap.xml written`);
console.log(`  - ${result.staticCount} static URLs`);
console.log(`  - ${result.categoryCount} service category URLs`);
console.log(`  - ${result.corridorCount} corridor URLs`);
console.log(`  - ${result.movingCityCount} anchor-city /moving-<slug> URLs`);
console.log(`  - ${result.crossBorderCount} cross-border /corridor URLs`);
