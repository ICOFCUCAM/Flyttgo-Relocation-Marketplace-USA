...
-- 3a. Schema reconciliation — the audit trigger (4b) and unified_ledger
--     view (5) reference columns that the original schema didn't have:
--       driver_subscriptions.plan   — TEXT slug joining subscription_tiers(slug)
--       bookings.booking_country    — TEXT ISO country code driving currency
--       escrow_payments.amount      — NUMERIC hold amount
--     Add them here so downstream sections compile.
ALTER TABLE public.driver_subscriptions
  ADD COLUMN IF NOT EXISTS plan TEXT
  REFERENCES public.subscription_tiers(slug);

CREATE INDEX IF NOT EXISTS driver_subscriptions_plan_idx
  ON public.driver_subscriptions (plan);

ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS booking_country TEXT;

CREATE INDEX IF NOT EXISTS bookings_booking_country_idx
  ON public.bookings (booking_country);
